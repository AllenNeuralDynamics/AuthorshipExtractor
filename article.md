...
The chord diagram arranges authors around a ring using spectral ordering: the widget computes the Fiedler vector (the eigenvector corresponding to the second-smallest eigenvalue of the collaboration graph's Laplacian) via power iteration with deflation, then sorts authors by their Fiedler values. This produces a ring ordering that minimizes total weighted edge length, placing heavily-connected authors adjacent and reducing visual clutter from crossing arcs. The collaboration layout uses a force-directed simulation with repulsive forces between all nodes and attractive spring forces proportional to collaboration weight (shared CRediT roles plus shared section contributions), converging over 300 iterations with velocity damping and collision resolution to position authors so that spatial proximity directly reflects collaboration strength. The UMAP layout applies the Uniform Manifold Approximation and Projection algorithm: it constructs a fuzzy simplicial set from k-nearest neighbors in collaboration-distance space, then optimizes a low-dimensional embedding via stochastic gradient descent with attractive and repulsive forces derived from cross-entropy between the high-dimensional and low-dimensional fuzzy sets. The spectral initialization and SGD optimization are implemented entirely in the browser with no external dependencies, making UMAP's cluster-revealing properties available without server-side computation.
