...
We propose a fundamentally different approach. Rather than flattening contributions into a single ranking, we offer an interactive explorer that surfaces the full dimensionality of each person's role. The tool leverages the CRediT (Contributor Roles Taxonomy) standard and augments it with per-section attribution, project timelines, and detailed author profiles. It provides two primary modes of exploration. First, a force-directed collaboration network positions authors so that spatial proximity reflects shared roles; clicking any node centers the view around that person's ego network, making their collaborations immediately legible. Second, a sortable author list lets readers dynamically reorder contributors by alphabetical order, number of CRediT roles, specific role contributions, project join date, or institutional affiliation — making explicit that author position is a viewing choice, not a fact.
