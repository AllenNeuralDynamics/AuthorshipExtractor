...
A central design principle is that there is no single correct view of a collaboration. The Sorted List makes this explicit for ordering: a reader interested in who led the conceptualization can sort by that role; a hiring committee can sort by Software; a student can sort by Methodology. The Collaboration network makes it explicit for structure: clicking any author recenters the graph around that person's connections, and clicking a role or institution in the legend reconfigures the view to highlight that subgroup. Every interaction reinforces the same message: contributions are multi-dimensional, and the reader chooses which dimension to foreground.
