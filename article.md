...
We propose a fundamentally different approach. Rather than flattening contributions into a single ranking, we offer an interactive explorer that surfaces the full dimensionality of each person's role. The tool leverages the CRediT (Contributor Roles Taxonomy) standard and augments it with per-section attribution, project timelines, and detailed author profiles. Six complementary views let readers examine contributions from different angles: a force-directed collaboration network where spatial proximity reflects shared roles and any node can be clicked to reveal an ego-centric view; a CRediT matrix showing contribution levels across all roles; section and timeline maps; a dynamically sortable author list that makes explicit how ordering is a viewing choice, not a fact; and individual author profiles.
