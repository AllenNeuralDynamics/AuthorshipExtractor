...
The **Sections** tab shows which authors contributed to the work described in each manuscript section and figure, enabling readers to identify the relevant expert for any part of the paper. This captures all forms of contribution—research, design, implementation, analysis—not just who wrote the text.
...
**Authorship documentation tools.** Tenzing {cite}`holcombe2019` is a web application and R package that lets teams document CRediT contributions via a shared spreadsheet. It generates several useful outputs: a prose contribution statement for insertion into a manuscript, structured affiliation blocks, JATS XML metadata suitable for publishers, YAML for the papaja R package, and funding acknowledgment and conflict-of-interest sections. Tenzing solves the data-collection problem well, but it is an input tool, not a display tool: its outputs are static text or metadata files, not interactive reader-facing components. There is no visualization, no contribution levels, no section-level attribution, and nothing that appears alongside the published article.
...
Fifth, and perhaps most consequentially, structured contribution data could evolve into a new de facto standard for scholarly attribution. Today the ordered author list is that standard, not because it is adequate but because nothing better has displaced it. The YAML format used in this framework offers a concrete alternative: a simple, human-readable, machine-parseable file that any researcher can create, any version-control system can track, and any tool can consume. If journals, preprint servers, and funding agencies converged on a common contribution schema, even a minimal one recording CRediT roles and contribution levels, it would unlock an entirely new layer of research evaluation. An author's ORCID profile could aggregate structured contribution records across all their publications, producing a contribution portfolio rather than a publication list. Hiring committees reviewing a candidate would no longer see a CV listing "Author 4 of 12" on a Nature paper; they would see that the candidate led Software and Data Curation across a body of work, supported Methodology in three collaborative projects, and contributed equally to Formal Analysis in two others. Grant agencies could assess whether an applicant has demonstrated the specific competencies a proposed project requires. Promotion boards could compare contribution fingerprints against the expectations for a given career stage, distinguishing a junior researcher who led Investigation from one who contributed to Writing—Review & Editing. None of this is possible when the only structured information available is a name and a position in a list. Standardizing the contribution record transforms authorship from an opaque ranking into a transparent, queryable career artifact.
