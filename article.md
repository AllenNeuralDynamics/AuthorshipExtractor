...
Not all contexts support interactivity. Printed papers, PDFs, and citation managers will continue to require a static representation. In these settings, we propose replacing the traditional ordered byline with a textual contribution summary, a string describing who did what, generated directly from the structured data. Such a summary carries less positional bias than a numbered name list while remaining compatible with any medium. The interactive display is the richest rendering; the same underlying record can equally produce a print-friendly table, a JATS XML block, or a plain-text contribution statement.
