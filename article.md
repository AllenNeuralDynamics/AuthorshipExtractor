...
Concretely, the system reduces conflict through two mechanisms: first, by making author position less consequential (every contribution is transparently documented regardless of byline order), and second, by removing the zero-sum dynamic of linear ranking (adding a contributor no longer dilutes anyone else's credit).
