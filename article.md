...
We propose a fundamentally different approach. Rather than flattening contributions into a single ranking, we offer an interactive explorer that surfaces the full dimensionality of each person's role. The tool leverages the CRediT (Contributor Roles Taxonomy) standard and augments it with per-section attribution, project timelines, and detailed author profiles. A force-directed collaboration network reveals shared-role connections at a glance, and clicking any author centers the view around their ego network. No fixed ordering is privileged: readers can re-sort by alphabetical order, number of CRediT roles, specific role contributions, project join date, or institutional affiliation.
